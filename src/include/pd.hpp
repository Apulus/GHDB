...
#define PD_RC_CHECK(rc,level,fmt,...)                                 \
   do {                                                               \
      PD_CHECK ( (GHDB_OK==(rc)), (rc), error, (level),                \
                 fmt, ##__VA_ARGS__) ;                                \
   } while ( 0 )                                                      \

#define GHDB_VALIDATE_GOTOERROR(cond,ret,str)                          \
   {if(!(cond)) { pdLog(PDERROR, __func__, __FILE__, __LINE__, str) ; \
    rc = ret; goto error ; }}

#ifdef _DEBUG
#define GHDB_ASSERT(cond,str)                                          \
   { if ( !(cond) ) {                                                 \
      pdassert ( str,__func__, __FILE__,__LINE__);}}                  \

#define GHDB_CHECK(cond,str)                                           \
   { if ( !(cond) ) {                                                 \
      pdcheck ( str, __func__, __FILE__, __LINE__ ) ; }}              \

#else
#define GHDB_ASSERT(cond,str)  {if(cond){}}
#define GHDB_CHECK(cond,str)   {if(cond){}}
#endif
