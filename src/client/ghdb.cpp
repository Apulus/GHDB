...
void Ghdb::start ( void ) {
	std::cout << "Welcome to GhDB Shell!" << std::endl ;
	std::cout << "ghdb help for help, Ctrl+c or quit to exit" << std::endl ;
	while ( 0 == gQuit ) {
		prompt () ;
	}
}
...
int Ghdb::readInput ( const char *pPrompt, int numIndent ) {
...
char *Ghdb::readLine ( char *p, int length ) {
	int len = 0 ;
	int ch ;
...
void Ghdb::split ( const std::string &text, char delim, std::vector<std::string> &result ) {
